...
class PixivClient:
    ...
    def _extract_cookies_native(cls, browser_name: str) -> List[dict]:
        """純粋 Python (+ pycryptodome + win32crypt) による Chrome/Edge の暗号化 Cookie 抽出エンジン"""
        import os
        import json
        import base64
        import shutil
        ...
        for prof in search_profiles:
            db_path = os.path.join(user_data_dir, prof, r"Network\Cookies")
            if not os.path.exists(db_path):
                db_path = os.path.join(user_data_dir, prof, "Cookies")
            if not os.path.exists(db_path):
                continue
    ...
    def auto_extract_browser_cookies(cls, target_file: str = "cookies.txt") -> dict:
        ...
        import os, glob, sqlite3, tempfile, shutil
        ...
        if not extracted_cookies:
            ...
            ff_profiles = glob.glob(os.path.join(app_data, r"Mozilla\Firefox\Profiles\*\cookies.sqlite"))
