...
適用方針:
- `BUNDLE` はグローバルシンボルとして提供される（PyInstallerがspecファイルを`exec`する際に自動注入するため）ため、`from PyInstaller.building.api import BUNDLE` のような明示的importは不要かつ環境によっては不整合の原因になるので行いません（MAC_PORTING.md記載のサンプルより単純化）。
...
適用方針: `flet` / `requests` / `Pillow` は既存コードで実際に使用されている（[main.py:1](main.py:1), [tray.py:2](tray.py:2), [pixiv_client.py](pixiv_client.py) 等）にもかかわらず現状の `requirements.txt` に記載がありません。macOSの新規クリーン環境で `pip install -r requirements.txt` のみでビルド可能にするため、本計画で明記します（絶対ルール①によりFletは `0.85.3` にバージョン固定）。
